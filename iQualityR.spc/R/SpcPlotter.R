...
SpcPlotter <- R6::R6Class("SpcPlotter",
  ...
  public = list(
    ...
    render = function(results, theme_obj, type = "full", plan = NULL, ...) {
      ...
      # Resolve ggplot2 theme
      gg_theme <- self$.resolve_theme(theme_obj)

      # Main chart
      p_main <- self$.plot_main(points_df, results, gg_theme, plan)
      ...
      if (type == "summary") {
        p_rules <- self$.plot_rules_table(results, gg_theme)
      ...
      if (chart_type %in% c("xbar_r", "xbar_s", "imr", "imr_rs") &&
        ...
        p_disp <- self$.plot_dispersion(
          results$data_tables$dispersion, results, gg_theme, chart_type)
    ...
    .plot_main = function(points_df, results, gg_theme, plan) {
      ...
      p <- ggplot2::ggplot(plot_data, ggplot2::aes(x = .data$x, y = .data$y)) +
        ...
        ggplot2::geom_rect(ggplot2::aes(xmin = -Inf, xmax = Inf,
          ...
          fill = "#FEF3C7", alpha = 0.3, inherit.aes = FALSE) +
        ggplot2::geom_rect(ggplot2::aes(xmin = -Inf, xmax = Inf,
          ymin = center - sigma, ymax = center + sigma),
          fill = "#D1FAE5", alpha = 0.3, inherit.aes = FALSE) +
        ...
        ggplot2::geom_hline(yintercept = lcl, linetype = "dashed",
          color = "#DC2626", linewidth = 0.8) +
        ggplot2::geom_hline(yintercept = center, color = "#2563EB",
          linewidth = 0.8) +
        # Data line and points
        ggplot2::geom_line(color = "#1F2937", linewidth = 0.6) +
        ggplot2::geom_point(color = "#1F2937", size = 2) +
        # Highlight violations
        ggplot2::geom_point(data = subset(points_df, is_violation),
          ggplot2::aes(x = index, y = value),
          color = "#DC2626", size = 3.5, shape = 19) +
        # Phase boundaries
        self$.phase_geom_vline(plan) +
    ...
    #' @param chart_type Chart type string.
    #' @return A ggplot object.
    .plot_dispersion = function(disp_df, results, gg_theme, chart_type) {
      plot_data <- data.frame(
    ...
    #' @param gg_theme ggplot2 theme.
    #' @return A ggplot object.
    .plot_rules_table = function(results, gg_theme) {
      ...
      ggplot2::ggplot(rules_df, ggplot2::aes(x = rule, y = n_violations)) +
        ggplot2::geom_col(fill = "#F59E0B", alpha = 0.85) +
    ...
    .phase_geom_vline = function(plan) {
      ...
      ggplot2::geom_vline(xintercept = boundaries, linetype = "dotdash",
        color = "#7C3AED", linewidth = 0.6, alpha = 0.6)
    },
