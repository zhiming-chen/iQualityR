...
plot_hypothesis_curve <- function(dist = c("norm", "t"),
  ...
  p <- p + ggplot2::geom_vline(
    xintercept = stat_value, color = "blue", linewidth = 1.2
  )

  # Critical value dashed lines
  if (alternative == "two.sided") {
    p <- p +
      ggplot2::geom_vline(xintercept = critical_value, color = "gray50", linetype = "dashed") +
      ggplot2::geom_vline(xintercept = -critical_value, color = "gray50", linetype = "dashed")
  } else if (alternative == "less") {
    p <- p + ggplot2::geom_vline(xintercept = -critical_value, color = "gray50", linetype = "dashed")
  } else {
    p <- p + ggplot2::geom_vline(xintercept = critical_value, color = "gray50", linetype = "dashed")
  }
  ...
  p <- p + ggplot2::annotate(
    ...
    vjust = 0, hjust = 0.5, size = 4, color = "blue", fontface = "bold"
  )
  ...
  if (!is.null(p_value)) {
    ...
    p <- p + ggplot2::annotate(
      ...
      label = p_label,
      vjust = 0, hjust = 0.5, size = 4, color = "gray20", fontface = "bold"
    )
  }
  ...
  if (alternative == "two.sided") {
    p <- p +
      ggplot2::annotate("text", x = -x_max * 0.7, y = max(y_seq) * 0.15,
                        label = "Rejection region", size = 3, color = "red") +
      ggplot2::annotate("text", x = x_max * 0.7, y = max(y_seq) * 0.15,
                        label = "Rejection region", size = 3, color = "red")
  } else if (alternative == "less") {
...
plot_hypothesis_box <- function(x, mu, sigma = NULL,
  ...
  p <- ggplot2::ggplot(data.frame(x = x), ggplot2::aes(x = 1, y = x)) +
    ggplot2::geom_boxplot(width = 0.2, fill = "steelblue", alpha = 0.3) +
    ggplot2::geom_jitter(width = 0.08, alpha = 0.3, size = 0.8, color = "gray40") +
    # H0 marker (red cross)
    ggplot2::annotate("point", x = 0.78, y = mu, size = 3, color = "red", shape = 13) +
    ggplot2::annotate("text", x = 0.72, y = mu, label = "H[0]", parse = TRUE,
                      size = 3.5, color = "red", fontface = "bold") +
    # Sample mean (blue point)
    ggplot2::annotate("point", x = 0.85, y = x_bar, size = 2.5, color = "blue") +
