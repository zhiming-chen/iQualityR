...
sigma_estimate <- function(x,
  ...
  # Aggregate by subgroup using base R (avoids data.table NSE dependency)
  g <- data_clean$g
  x <- data_clean$x
  ug <- unique(g)
  stats <- data.frame(
  ...
  stats$sub_mean <- as.numeric(tapply(x, g, mean, na.rm = TRUE))
  stats$R <- as.numeric(tapply(x, g, function(y) if (length(y) > 1) diff(range(y, na.rm = TRUE)) else NA))
  stats$S <- as.numeric(tapply(x, g, function(y) if (length(y) > 1) stats::sd(y, na.rm = TRUE) else NA))
...
.compute_sigma_core <- function(stats, method, use_unbiased, m_span) {
  switch(method,
    ...
    "mr_bar" = {
      ...
      # Moving range of span m: for each window of m consecutive values,
      # compute max - min. Previously used diff(lag = m_span-1) which only
      # gives the difference between the first and last point of the window,
      # not the true range.
      mr <- sapply(seq_len(length(means) - m_span + 1), function(i) {
        diff(range(means[i:(i + m_span - 1)]))
      })
