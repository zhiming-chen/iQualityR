...
compare_fits <- function(fit_results, alpha = 0.05) {
  ...
  for (dist_name in names(fit_results)) {
    ...
    ks_result <- tryCatch({
      cdf_func <- get(sprintf("p%s", dist), mode = "function", envir = asNamespace("stats"))
      stats::ks.test(x, cdf_func, unlist(params))
    }, error = function(e) NULL)
  ...
  ks_stats <- sapply(test_results, function(r) {
    if (!is.null(r$ks_test)) r$ks_test$statistic else Inf
  })
  best_by_ks <- names(which.min(ks_stats))

  ad_stats <- sapply(test_results, function(r) {
    if (!is.null(r$ad_test)) r$ad_test$statistic else Inf
  })
  best_by_ad <- names(which.min(ad_stats))

  list(
    test_results = test_results,
    best_by_ks = best_by_ks,
    best_by_ad = if (best_by_ad != "Inf") best_by_ad else NA,
...
calc_qq_data <- function(x, dist = "norm", params = list()) {
  ...
  probs <- (1:n - 0.5) / n
  qfunc <- get(sprintf("q%s", dist), mode = "function", envir = asNamespace("stats"))
...
private_fit_gamma <- function(x, n, start = NULL) {
  ...
  if (requireNamespace("MASS", quietly = TRUE)) {
    fit <- tryCatch(
      MASS::fitdistr(x, "gamma", start = start),
      error = function(e) NULL
    )
    if (!is.null(fit)) {
...
private_fit_beta <- function(x, n, start = NULL) {
  ...
  if (requireNamespace("MASS", quietly = TRUE)) {
    fit <- tryCatch(
      MASS::fitdistr(x, "beta", start = start),
      error = function(e) NULL
    )
    if (!is.null(fit)) {
...
private_fit_cauchy <- function(x, n, start = NULL) {
  ...
  if (requireNamespace("MASS", quietly = TRUE)) {
    fit <- tryCatch(
      MASS::fitdistr(x, "cauchy", start = start),
      error = function(e) NULL
    )
    if (!is.null(fit)) {
...
private_fit_t <- function(x, n, start = NULL) {
  ...
  if (requireNamespace("MASS", quietly = TRUE)) {
    fit <- tryCatch(
      MASS::fitdistr(x, "t", start = start),
      error = function(e) NULL
    )
    if (!is.null(fit)) {
      df <- fit$estimate["df"]
      location <- fit$estimate["mu"]
      scale <- fit$estimate["sigma"]
    }
  }
