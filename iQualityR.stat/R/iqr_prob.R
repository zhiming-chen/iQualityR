...
#' prob_calc(type = "norm", params = list(mean = 0, sd = 1), x = 1.96)
#' }
#'
#' @export
iqr_prob <- R6::R6Class("iqr_prob",
  public = list(
    ...
    #' prob$plot(facet = TRUE)        # Faceted display (multi-distribution)
    #' }
    ...
    report_excel = function(path = NULL, excel_exporter = NULL) {
      ...
      # If ExcelExporter not provided, try to create
      if (is.null(excel_exporter)) {
        if (exists("ExcelExporter")) {
          if (!is.null(self$theme_obj)) {
            excel_exporter <- ExcelExporter$new(self$theme_obj$config)
          } else {
            excel_exporter <- ExcelExporter$new()
          }
        } else {
          stop("[iqr_prob] ExcelExporter class not found, please install iQualityR reporting module.",
               call. = FALSE)
        }
      }
