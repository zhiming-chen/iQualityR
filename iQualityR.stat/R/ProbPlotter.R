...
ProbPlotter <- R6::R6Class("ProbPlotter",
  ...
  public = list(
    ...
    #' @param theme_obj IqrTheme object (overrides `self$theme_obj`)
    #' @return ggplot/patchwork object
    render = function(nodes, calc_results, facet, show_cdf, mode, theme_obj = NULL) {
      effective_theme <- if (!is.null(theme_obj)) theme_obj else self$theme_obj
      ...
      # 3. Build PDF plot
      p1 <- private$.build_pdf_plot(plot_data, calc_results, facet, themeobj = effective_theme)

      # 4. Determine if CDF plot is needed
      must_show_cdf <- show_cdf || any(sapply(calc_results, function(r) !r$all_res[[1]]$is_prob_mode))

      if (must_show_cdf) {
        p2 <- private$.build_cdf_plot(plot_data, calc_results, nodes, facet, effective_theme)
  ...
  private = list(
    ...
    .build_plot_data = function(nodes, xr, calc_results) {
      do.call(rbind, lapply(names(nodes), function(id) {
        ...
        d <- data.frame(x = x_s, group = id, is_discrete = node$is_discrete)
        d$pdf <- sapply(x_s, node$d)
        d$cdf <- sapply(x_s, node$p)
        d$in_shade <- factor(calc_results[[id]]$shade_f(x_s), levels = c("TRUE", "FALSE"))
        d$pdf_shade <- ifelse(d$in_shade == "TRUE", d$pdf, NA)
        d
      }))
    },
    ...
    .build_pdf_plot = function(plot_data, calc_results, facet, themeobj = NULL) {
      ...
      if (any(plot_data$is_discrete)) {
        p <- p + ggplot2::geom_col(
          data = subset(plot_data, is_discrete),
      ...
      if (any(!plot_data$is_discrete)) {
        p <- p +
          ggplot2::geom_line(data = subset(plot_data, !is_discrete), linewidth = 1) +
          ggplot2::geom_ribbon(
            data = subset(plot_data, !is_discrete),
