...
private_rule_3 <- function(x, center, sigma) {
  ...
  # A window of 6 points produces 5 consecutive differences; all 5 must
  # share the same sign (strict monotonicity).
  if (n >= 6) for (i in 6:n) {
    window <- x[(i - 5):i]
    diffs <- diff(window)
...
private_rule_4 <- function(x, center, sigma) {
  ...
  # Rule 4: 14 consecutive points alternating up and down.
  # A window of 14 points produces 13 consecutive differences whose
  # signs must strictly alternate between +1 and -1 (no flat segment).
  if (n >= 14) for (i in 14:n) {
    window <- x[(i - 13):i]
    diffs <- diff(window)
    signs <- sign(diffs)

    # Strict alternation: no zero differences and consecutive signs flip.
    if (all(signs != 0) && all(abs(diff(signs)) == 2)) {
      violations <- c(violations, i)
    }
  }
