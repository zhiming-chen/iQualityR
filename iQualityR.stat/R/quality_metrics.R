...
quality_dashboard <- function(cpk = NULL, ppm = NULL, yield_val = NULL,
  ...
  # Normalize percentage inputs (yield_to_dpmo auto-detects >1 as percentage;
  # apply the same convention here so displays and benchmarks stay consistent).
  if (!is.null(yield_val) && yield_val > 1) yield_val <- yield_val / 100
  if (!is.null(availability) && availability > 1) availability <- availability / 100
  ...
  if (!is.null(cpk)) {
    ...
    result$ppm_from_cpk <- capability_to_ppm(cpk = cpk, usl = 1, lsl = 0)
  }
