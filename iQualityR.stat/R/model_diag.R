...
test_residual_normality <- function(model, method = "auto") {
  if (!inherits(model, "lm")) {
    stop("model must be an lm object.")
  }
  ...
  result <- switch(method,
    "sw" = list(
      ...
      is_normal = stats::shapiro.test(res)$p.value >= 0.05
    ),
    "ad" = {
...
test_heteroscedasticity <- function(model, test = c("bp", "ncv", "white")) {
  if (!inherits(model, "lm")) {
    stop("model must be an lm object.")
  }
  ...
  switch(test,
    "bp" = private_bp_test(model),
    "ncv" = private_ncv_test(model),
    "white" = private_white_test(model)
  )
}
...
private_heteroscedasticity_test <- function(model, res, fitted, alpha) {
  private_bp_test(model)
}

private_bp_test <- function(model) {
  res <- residuals(model)
  fitted <- fitted.values(model)
  n <- length(res)
...
private_ncv_test <- function(model) {
  res <- residuals(model)
  fitted <- fitted.values(model)
  n <- length(res)
  ...
  list(
    ...
    statistic = ncv_stat,
    p.value = ncv_p,
    is_heteroscedastic = ncv_p < 0.05
  )
}

private_white_test <- function(model) {
  res <- residuals(model)
  fitted <- fitted.values(model)
  n <- length(res)
  ...
  list(
    ...
    p.value = white_p,
    is_heteroscedastic = white_p < 0.05
  )
}
...
private_dw_test <- function(res) {
  ...
  p_value <- if (dw_stat < 2) {
    2 * stats::pnorm(-sqrt(n) * abs(dw_stat - 2) / sqrt(2))
  } else {
    2 * (1 - stats::pnorm(sqrt(n) * abs(dw_stat - 2) / sqrt(2)))
  }
