...
#'
#' @examples
#' # Johnson transform requires the SuppDists Suggests package
...
auto_transform <- function(x, methods = c("box_cox", "yeo_johnson", "log", "sqrt", "reciprocal"),
  ...
  # Select best transformation by the chosen criterion. The previous code
  # always used normality_after (Shapiro-Wilk p-value) regardless of criterion.
  if (criterion == "shapiro") {
    # Maximize Shapiro-Wilk p-value (higher = more normal)
    scores <- sapply(results, function(r) r$normality_after)
    best_idx <- which.max(scores)
  } else {
    # Minimize Anderson-Darling statistic (lower = more normal)
    scores <- sapply(results, function(r) {
      unname(nortest::ad.test(r$transformed)$statistic)
    })
