...
#' @export
#' @examples
#' # Register a triangular-shaped custom distribution (then unregister it)
...
register_dist <- function(dist_type, d_func, p_func, q_func,
  ...
  # Unlock the binding so the registry can be extended at runtime (the
  # package namespace is locked after load; relock afterwards to preserve
  # the invariant for downstream code).
  unlockBinding("DIST_REGISTRY", asNamespace("iQualityR.stat"))
  on.exit(lockBinding("DIST_REGISTRY", asNamespace("iQualityR.stat")))
