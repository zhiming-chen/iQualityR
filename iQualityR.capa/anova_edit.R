...
AnovaPlotter <- R6::R6Class("AnovaPlotter",
  ...
  public = list(
    ...
    plot_residuals = function(result, add_qq = TRUE, ...) {
      ...
      p <- (p_rf + p_qq) / (p_sl + p_factor) +
        patchwork::plot_annotation(
          ...
          theme = iQualityR.plot::patchwork_annotation_theme(self$theme_obj)
        ) + patchwork::plot_layout(guides = "collect") +
        patchwork::plot_spacing(grid::unit(0.3, "cm")) &
        iQualityR.plot::patchwork_panel_theme(self$theme_obj) &
    ...
    #' @param result ANOVA result.
    #' @param layout Layout: `"2x2"` or `"1x3"`. Both now use the same unified
    #'   2-row layout to avoid nesting the 4-panel residual diagnostics into a
    #'   single cell, which made the output unreadable.
    ...
    plot_summary = function(result, layout = "2x2", ...) {
      ...
      table_grob <- iQualityR.plot::create_anova_table(result[["anova_table"]],
                                                       theme = self$theme_obj)
      p4 <- ggplot2::ggplot() +
        ggplot2::annotation_custom(table_grob, xmin = 0, xmax = 1,
                                   ymin = 0, ymax = 1) +
        ggplot2::theme_void()

      # Right panel: interaction (if multi-factor) else ANOVA table
      right_panel <- p3 %||% p4

      # Unified 2-row layout:
      #   Row 1: effects + (interaction | table)  -- 2 cols
      #   Row 2: residual 4-panel diagnostics       -- full width
      # This avoids the critical "nesting" bug where the 4-panel residual
      # was crammed into a single cell of a 2x2 grid, making it unreadable.
      # The residual row gets 2.0× the height of the top row because it
      # contains a 2×2 grid of diagnostic plots that need room to breathe.
      design <- "
AABB
CCCC
"
      p <- (p1 + right_panel + p2) +
        patchwork::plot_layout(design = design, heights = c(1, 2.0))
