...
#' @export
#' @examples
#' # Descriptive stats for a sample
#' set.seed(123)
...
desc_hist <- function(s, theme = "academic") {
  ...
  p <- iQualityR.plot::base_plot(df, ggplot2::aes(x = v), theme = theme) +
    iQualityR.plot::layers_histogram_density(
      bins = 20,
...
desc_box <- function(s, theme = "academic") {
  ...
  p <- iQualityR.plot::base_plot(df, ggplot2::aes(x = v, y = ""), theme = theme) +
    iQualityR.plot::layers_boxplot(
      add_jitter = TRUE,
...
desc_box_with_stats <- function(s, theme = "academic") {
  ...
  p <- iQualityR.plot::base_plot(df, ggplot2::aes(x = 1, y = v), theme = theme) +
    iQualityR.plot::layers_boxplot(
      add_jitter = TRUE,
...
desc_plot <- function(s, theme = "academic") {
  ...
  p_hist + p_box_stats + patchwork::plot_layout(widths = c(1, 1.3)) +
    patchwork::plot_annotation(
      title = sprintf("Descriptive Analysis: %s", s$var_name %||% "Variable"),
      theme = iQualityR.plot::patchwork_annotation_theme(theme)
    )
}
...
desc_stats_table <- function(s, theme = "academic") {
  ...
  p <- iQualityR.plot::base_plot(stats_data, ggplot2::aes(x = metric, y = value), theme = theme) +
