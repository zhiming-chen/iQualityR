...
plot_anova_summary <- function(model,
    ...
    # Unified 2-row design to avoid nesting the 4-panel residual plot (p3)
    # into a single cell of a 2x2 grid, which made it unreadable.
    # Row 1: effects + (interaction or table)  -- 2 cols
    # Row 2: residual diagnostics               -- full width
    right_panel <- p2 %||% p4 %||% (ggplot2::ggplot() + ggplot2::theme_void())
    if (is.null(p1)) p1 <- ggplot2::ggplot() + ggplot2::theme_void()
    if (is.null(p3)) p3 <- ggplot2::ggplot() + ggplot2::theme_void()
    design <- "
AABB
CCCC
"
